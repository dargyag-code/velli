'use client';
import React, { useState, useEffect } from 'react';
import Link from 'next/link';
import { usePathname } from 'next/navigation';
import { ArrowLeft } from 'lucide-react';

const STYLIST_KEY = 'velli_stylist_name';

interface HeaderProps {
  showBack?: boolean;
  title?: string;
  rightAction?: React.ReactNode;
}

export default function Header({ showBack, title, rightAction }: HeaderProps) {
  const pathname = usePathname();
  // null = aún no leído (evita mismatch SSR/cliente)
  const [stylistName, setStylistName] = useState<string | null>(null);

  useEffect(() => {
    setStylistName(localStorage.getItem(STYLIST_KEY) ?? '');
  }, []);

  // ── Header de detalle / wizard (con botón atrás) ────────────────────────
  if (showBack) {
    return (
      <header
        className="sticky top-0 z-40 bg-white/95 backdrop-blur-sm border-b border-[#E5E5E5] px-4 py-3"
        style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}
      >
        <div className="flex items-center gap-3 max-w-2xl mx-auto">
          <Link
            href={pathname.startsWith('/clientas/') ? '/clientas' : '/'}
            className="p-2 rounded-xl hover:bg-[#EEF5ED] transition-colors text-[#2D5A27]"
          >
            <ArrowLeft size={20} />
          </Link>
          <h1
            className="text-base text-[#2D2D2D] flex-1 truncate"
            style={{ fontFamily: "var(--font-dm-serif), 'DM Serif Display', serif" }}
          >
            {title}
          </h1>
          {rightAction}
        </div>
      </header>
    );
  }

  // ── Header principal (dashboard, listas, secciones) ─────────────────────
  const saludo = stylistName ? `Hola, ${stylistName}` : stylistName === '' ? 'Hola' : null;

  return (
    <header
      className="sticky top-0 z-40 bg-white/95 backdrop-blur-sm border-b border-[#E5E5E5]"
      style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}
    >
      <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
        {/* Logo */}
        <Link href="/" className="flex flex-col leading-none gap-0.5">
          <span
            className="text-2xl text-[#2D5A27] tracking-tight"
            style={{ fontFamily: "var(--font-dm-serif), 'DM Serif Display', serif" }}
          >
            Velli
          </span>
          <span
            className="text-[9px] font-semibold text-[#C9956B] tracking-widest uppercase"
            style={{ fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif", letterSpacing: '0.12em' }}
          >
            Inteligencia capilar
          </span>
        </Link>

        {/* Saludo — solo se muestra después de leer localStorage */}
        {saludo !== null && (
          <div
            className="text-sm text-[#666666] flex items-center gap-1"
            style={{ fontFamily: "var(--font-dm-sans), 'DM Sans', sans-serif" }}
          >
            <span>{saludo}</span>
            <span role="img" aria-label="planta">🌿</span>
          </div>
        )}
      </div>
    </header>
  );
}
