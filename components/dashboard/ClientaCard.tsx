...
export default function ClientaCard({ clienta }: ClientaCardProps) {
  ...
  return (
    <Link href={`/clientas/${clienta.id}`}>
      <div className="flex items-center gap-3 p-3 bg-white rounded-2xl border border-[#E5E5E5] hover:border-[#C4A0E8] hover:shadow-sm transition-all duration-200 card-hover">
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          {clienta.totalVisitas > 0 && (
            <span
              className="text-xs font-bold bg-[#F3EDF9] text-[#5B2D8E] px-2 py-0.5 rounded-full"
              style={{ fontFamily: "'Montserrat', sans-serif" }}
            >
